...
namespace ChatRoomAPI.Controllers.Back_Side
{
    ...
    [ApiController]
    public class GetPageDetailController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public GetPageDetailController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpGet]
        public IActionResult GetPageDetail([FromQuery] string account)
        {
            ...
            try
            {
                ...
                GetPageDetail data = new GetPageDetail();
                List<GetPageComponent> pageComponents = new List<GetPageComponent>();
                ...
                ResponseGetPageDetail responseData = new ResponseGetPageDetail()
                {
                    resultCode = "10",
                    data = data,
                    message = "取得會員管理明細成功！"
            ...
            catch (Exception ex)
            {
                ...
                {
                    resultCode = "01",
                    errorMessage = $"GetPageDetail Error： {ex.Message}，請聯繫開發人員！"
                };
